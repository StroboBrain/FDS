...
class Node:
    ...
    def run(self):
        while True:
            ...
            if self.working:
                now = time.time()
                #leader only sends heartbeats
                if self.state == 'leader':
                    if now - self._lastHbSent >= HEARTBEAT_INTERVAL:
                        self.broadcast('heartbeat', {'leader_id': self.id})
                        self._lastHbSent = now
                #follower: if no heartbeat received start a random backoff, to delay my candidacy. Then if my deadline is due and i haven't seen other candidates during my backoff, become candidate, announce candidacy, self-vote, and open the 2s vote window.
                    ...
                    if now - self.lastHeartbeat > HEARTBEAT_TIMEOUT:
                        ...
                        if self.backoffDeadline is not None and now >= self.backoffDeadline and not self.seenForeignCandidacy:
                            ...
                            self.voteCount += 1
                # candidate: when the 2s election window expires, if voteCount >= majority(len(nodes))
                # I become leader and announce it; otherwise I revert to follower and reset state
                # (refresh lastHeartbeat to avoid instant re-election).
    ...
    def crash(self):
        if self.working:
            self.working = False
            buffer[self.id] = []
            self.state = 'crashed' #set state to crashed?
    
    #if a node recovers, set its state to follower, so if a leader crashes and he recovers, we don't have 2 leaders
    def recover(self):
        if not self.working:
            buffer[self.id] = []
            self.working = True
    ...
    def deliver(self, msg_type, value):
        ...
        if msg_type == 'heartbeat':
            ...
            if self.state in ('follower','candidate'):
                ...
                self.backoffDeadline = None
                self.seenForeignCandidacy = False
        elif msg_type == 'candidacy': #voting for sender of the first candidacy message.
